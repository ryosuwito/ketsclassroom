...
const SurveyList = () => {
  ...
  useEffect(() => {
    ...
    fetch("http://localhost:8080/api/responder-groups/"+limit+"/"+(currentPage-1)).then(res=>res.json()).then(
      (result)=>{
    ...
    <CRow>
      <CCol xl={12}>
        <CCard>
          <CCardHeader>
            Kelas
            <Link to="/responder-group/create">
              <CButton className="float-right" color="primary">Tambah Kelas</CButton>
            </Link>
          </CCardHeader>
          <CCardBody>
          <CDataTable
            ...
            activePage={page}
            clickableRows
            onRowClick={(item) => history.push(`/responder-group/${item.id}`)}
          />
          <CPagination
            activePage={page}
